...
export const changelog: ChangelogEntry[] = [
  {
    ...
    "源站寻片增加服务过滤搜索",
    "短剧增加二级分类识别与返回快照",
    "观影室进度同步类型支持倍数同步，支持播放页一键创建观影室，新增邀请链接",
    "外部观影室支持使用appid实现多服务共用",
    "漫画指定页数跳转",
    "详情面板增加剧照",
    "详情面板增加纠错",
    "新增优选偏好",
    ...
    changed: [
    "ass/ssa/srt/vtt字幕优先",
    "OpenList 115 播放直链改为由浏览器端解析",
    "美化电子书库，漫画展馆，音乐",
    "全新的初始化加载动画、评分ui",
    "管理面板在PC下改为左右结构",
    "详情面板使用年份和集数辅助识别",
    "webtv打包移除Android过时的tools包"
    ],
    fixed: [
    "修复源站寻片因返回快照导致无法翻页",
    "修复自动匹配重复加载弹幕",
    ...
    "修复观影室房主刷新单人房间解散问题",
    "修复观影室房主暂停，房员不同步问题",
    "修复漫画页数指示无法到达最后一页，修复漫画垂直和水平滚动无下一页指示",
    "修复保存通知设置按钮间距问题",
    "修复详情面板季度获取失败不显示季度",
    "修复弹幕热力图首屏不显示、换集不清空问题"
    ],
    security: [
      // 无安全修复内容
    ],
  ...
  },
  {
    ...
    date: "2026-09-12",
    added: [
    "所有弹幕源都没有当前集数弹幕时，在播放器右上角弹幕数量位置提示“未匹配到弹幕”"
    ],
    changed: [
    "弹幕源改为自动级联匹配：初次播放不再弹出选源框，按“上次成功的源优先，其余按搜索结果顺序”依次尝试",
    "优先采用集数严格匹配且确实有弹幕的源；严格匹配的源都不可用时，才退回索引兜底匹配",
    "自动匹配成功的源会被记住并优先用于后续集数，该记忆不覆盖手动选择的弹幕源",
    "弹幕插件就绪前不再发起弹幕源搜索，避免浪费弹幕源接口"
    ],
    fixed: [
  ...
  },
  {
    ...
    date: "2026-02-14",
    added: [
    "新增弹幕集数智能匹配，支持多种格式（Emby格式S01E03、EP格式、第X集、第X话、第X部、特别篇、SP、OVA等）",
    "新增弹幕源自动切换功能，当主弹幕源某集匹配失败时自动尝试候选弹幕源",
    "新增弹幕候选源记忆，记住用户选择的弹幕源用于后续集数匹配",
    "新增播放源搜索多级宽松匹配策略，解决因年份/标题细微差异导致匹配失败的问题"
    ],
    changed: [
    "弹幕自动加载优化，不再弹出选择框，直接使用第一个弹幕源",
    "所有弹幕源都匹配不到时弹出选择框让用户选择",
    "换源后自动清除弹幕记忆并重新搜索"
    ],
    fixed: [
  ...
  },
  {
    ...
    "新增 Anime4K 超分设置云端同步",
    "新增跳过片头片尾时间云端同步，同名视频跨来源共享跳过时间",
    "新增豆瓣评论 API 代理支持，复用现有 CMLiussss 代理配置"
    ],
    changed: [
    "源站寻片页面布局优化，分类下拉框保持固定显示",
    "移除“选择视频源”和“乐子资源的分类”提示标签，使用颜色深度区分",
    "搜索框移至顶部，源选择器和分类下拉框合并",
    "源站寻片来源和分类选择器放在同一背景方框内",
    "源站寻片搜索逻辑优化，切换源时保留搜索关键词自动搜索",
    "源站寻片搜索框和选项卡颜色优化",
    "轮播图预告片声音开关按钮移到右下角，避免被覆盖",
    "轮播图预告片优化，首次加载后 2 秒才开始请求视频",
    "轮播图切换间隔改为 22 秒，切换时暂停上一张视频"
    ],
    fixed: [
    "解决 PWA 构建超时问题",
    "PWA 缓存配置优化",
