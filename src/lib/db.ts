...
class NoOpStorage implements IStorage {
  ...
  async deletePlayRecords(userName: string, keys: string[]): Promise<void> { return this.throwError('deletePlayRecords'); }
...
function getD1Adapter(): any {
  ...
  // 检查是否为 Cloudflare 构建（使用更安全的方式检测）
  const isCloudflare = typeof process !== 'undefined' && process.env && (process.env.CF_PAGES === '1' || process.env.BUILD_TARGET === 'cloudflare');
