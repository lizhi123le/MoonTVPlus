...
function PlayPageClient() {
  ...
  const handleSourceChange = async (
    ...
    try {
      ...
      // 换源后优先使用保存的弹幕源，而不是清除记忆重新搜索
      const title = newDetail.title || newTitle;
      if (title) {
        console.log('[换源] 标题:', title);
        
        // 先检查是否有保存的弹幕源，如果有则直接使用
        const savedAnimeId = getDanmakuAnimeId(title);
        if (savedAnimeId) {
          console.log('[换源] 找到保存的弹幕源:', savedAnimeId);
          // 延迟后使用保存的弹幕源加载
          setTimeout(async () => {
            // 直接使用保存的弹幕源（不使用autoSearchDanmaku，避免重新搜索）
            await loadSavedDanmaku(title, savedAnimeId);
          }, 500);
        } else {
          // 没有保存的弹幕源，才执行自动搜索
          console.log('[换源] 没有保存的弹幕源，执行自动搜索');
          setTimeout(async () => {
  ...
  const handleDanmakuSourceSelect = async (selectedAnime: DanmakuAnime, selectedIndex?: number, isManualSearch = false) => {
    ...
    try {
      ...
      // 保存多个候选弹幕源（用于匹配失败时自动尝试下一个）
      // 从 danmakuMatches 中获取所有搜索结果
      if (title && danmakuMatches.length > 1) {
        ...
        if (candidates.length > 0) {
          ...
          console.log(`[弹幕记忆] 保存了 ${candidates.length} 个候选弹幕源: ${candidates.join(', ')}`);
        }
      }

      // 递归尝试加载弹幕源，支持失败时自动切换候选
      const tryLoadWithFallback = async (animeList: DanmakuAnime[], isFallback = false): Promise<boolean> => {
        if (animeList.length === 0) {
          console.log('[弹幕] 所有候选弹幕源都已尝试完毕');
          return false;
        }

        const anime = animeList[0];
        const remaining = animeList.slice(1);

        console.log(`[弹幕] ${isFallback ? '尝试候选' : '使用主'}弹幕源: ${anime.animeTitle}`);
        ...
        try {
          ...
          if (episodesResult.success && episodesResult.bangumi.episodes.length > 0) {
            ...
            if (episode) {
              ...
              console.log(`[弹幕] ${anime.animeTitle} 集数匹配失败，尝试下一个候选`);
              // 匹配失败，尝试下一个候选
              return await tryLoadWithFallback(remaining, true);
            }
          } else {
            console.log(`[弹幕] ${anime.animeTitle} 获取剧集列表失败，尝试下一个候选`);
            return await tryLoadWithFallback(remaining, true);
          }
        } catch (error) {
          console.error(`[弹幕] ${anime.animeTitle} 加载失败:`, error);
          return await tryLoadWithFallback(remaining, true);
        }
      };

      // 构建候选列表（当前选择 + 其他候选）
      const candidateList = [selectedAnime, ...danmakuMatches.filter(m => m.animeId !== selectedAnime.animeId)];
      
      // 开始尝试加载弹幕
      const success = await tryLoadWithFallback(candidateList);

      if (!success) {
        console.warn('[弹幕] 所有弹幕源都匹配失败');
        if (artPlayerRef.current) {
          artPlayerRef.current.notice.show = '未找到匹配的弹幕，请尝试其他弹幕源';
        }
      }
    } catch (error) {
      console.error('加载弹幕失败:', error);
      if (artPlayerRef.current) {
        artPlayerRef.current.notice.show = '弹幕加载失败，请稍后重试';
      }
    } finally {
      setDanmakuLoading(false);
    }
  ...
  // 使用保存的弹幕源加载（换源时优先使用）
  const loadSavedDanmaku = async (title: string, savedAnimeId: number) => {
    if (isDirectPlay) return;
    const disableAutoLoad = localStorage.getItem('disableAutoLoadDanmaku') === 'true';
    if (disableAutoLoad) return;
    ...
    console.log('[弹幕] 使用保存的弹幕源 - 标题:', title, 'animeId:', savedAnimeId, '集数:', currentEpisodeIndex);
    ...
    try {
      ...
      if (!episodesResult.success || !episodesResult.bangumi.episodes || episodesResult.bangumi.episodes.length === 0) {
        console.log('[弹幕] 保存的弹幕源获取剧集列表失败，尝试候选源');
        // 尝试候选源
        const candidates = getDanmakuAnimeCandidates(title);
        if (candidates && candidates.length > 0) {
          const tryCandidate = async (candidateList: number[]): Promise<boolean> => {
            if (candidateList.length === 0) return false;
            
            const nextAnimeId = candidateList[0];
            const remaining = candidateList.slice(1);
            
            console.log('[弹幕] 尝试候选源:', nextAnimeId);
            
            try {
              const result = await getEpisodes(nextAnimeId);
              if (result.success && result.bangumi.episodes.length > 0) {
                const videoEpTitle = detailRef.current?.episodes_titles?.[currentEpisodeIndex];
                const matchedEpisode = matchDanmakuEpisode(currentEpisodeIndex, result.bangumi.episodes, videoEpTitle);
                ...
                if (matchedEpisode) {
                  ...
                  await handleDanmakuSelect(selection, false);
                  console.log('[弹幕] 候选源匹配成功:', result.bangumi.animeTitle);
                  return true;
                } else {
                  return await tryCandidate(remaining);
                }
              } else {
                return await tryCandidate(remaining);
              }
            } catch (error) {
              console.error('[弹幕] 候选源加载失败:', error);
              return await tryCandidate(remaining);
            }
          };
          
          const found = await tryCandidate(candidates);
          if (!found) {
            console.log('[弹幕] 所有候选源都失败，执行自动搜索');
            await autoSearchDanmaku(title);
          }
        } else {
          // 没有候选源，执行自动搜索
          console.log('[弹幕] 没有候选源，执行自动搜索');
          await autoSearchDanmaku(title);
        }
        return;
      }
      ...
      // 尝试匹配当前集数
      const videoEpTitle = detailRef.current?.episodes_titles?.[currentEpisodeIndex];
      const episode = matchDanmakuEpisode(currentEpisodeIndex, episodesResult.bangumi.episodes, videoEpTitle);
      ...
      if (episode) {
        ...
        setDanmakuEpisodesList(episodesResult.bangumi.episodes);
        console.log('[弹幕] 使用保存的弹幕源匹配成功:', selection);
        
        await handleDanmakuSelect(selection, false);
      } else {
        console.log('[弹幕] 保存的弹幕源集数匹配失败，尝试候选源');
        // 集数匹配失败，尝试候选源
        const candidates = getDanmakuAnimeCandidates(title);
        if (candidates && candidates.length > 0) {
          const tryCandidate = async (candidateList: number[]): Promise<boolean> => {
      ...
      console.error('[弹幕] 使用保存的弹幕源失败:', error);
      // 失败后执行自动搜索
      await autoSearchDanmaku(title);
    }
  };
  ...
  const autoSearchDanmaku = async (overrideTitle?: string) => {
    ...
    try {
      ...
      if (searchResult.success && searchResult.animes.length > 0) {
        ...
        // 直接使用第一个弹幕源，匹配失败时自动尝试候选
        const tryLoadDanmaku = async (animeList: DanmakuAnime[], isFallback = false): Promise<boolean> => {
          if (animeList.length === 0) {
            return false;
          }
          ...
          console.log(`[弹幕] ${isFallback ? '候选' : '主'}弹幕源: ${anime.animeTitle}`);
          ...
          try {
            ...
            if (episodesResult.success && episodesResult.bangumi.episodes.length > 0) {
              ...
              if (episode) {
                ...
                // 通过 handleDanmakuSelect 处理弹幕加载
                await handleDanmakuSelect(selection, !isFallback);
                return true;
              } else {
                console.log(`[弹幕] ${anime.animeTitle} 集数匹配失败，尝试下一个`);
                // 尝试下一个候选
                return await tryLoadDanmaku(remaining, true);
              }
            } else {
              console.log(`[弹幕] ${anime.animeTitle} 获取剧集列表失败`);
              return await tryLoadDanmaku(remaining, true);
            }
          } catch (error) {
            console.error(`[弹幕] ${anime.animeTitle} 加载失败:`, error);
              ...
              <div className='md:col-span-4 lg:col-span-5'>
                <div className='p-6 flex flex-col min-h-0'>
                  ...
                  <div className={`flex flex-wrap items-center gap-3 text-base mb-4 opacity-80 flex-shrink-0 ${tmdbBackdrop ? 'text-white' : ''}`}>
                    ...
                    {detail?.source_name && (
                      <span className={`border px-2 py-[1px] rounded ${detail.source === 'xiaoya' ? 'border-blue-500' : detail.source === 'openlist' || detail.source === 'emby' || detail.source?.startsWith('emby_') ? 'border-yellow-500' : 'border-gray-500/60'
                        }`}>
                        {detail.source_name}
                      </span>
                    )}
