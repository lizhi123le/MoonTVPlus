      ...
      try {
        ...
        if (data.code === 200 && Array.isArray(data.data)) {
          ...
          if (list.length > 0) {
            ...
            // 其次是上次使用并保存在 localStorage 的来源
            const savedSource = restoreSourceFromStorage();
            const isAvailable = (key: string) =>
              !!key && list.some((item) => item.key === key);

            setSelectedSource((prev) =>
              prev && isAvailable(prev)
                ? prev
                : isAvailable(savedSource)
                  ? savedSource
                  : list[0].key
            );

            // 快照恢复的分类优先保留，否则先用采集源标记的短剧分类拉列表
            const preferredTypeId =
              (isAvailable(savedSource)
                ? list.find((item) => item.key === savedSource)?.typeId
                : '') ||
              list[0].typeId ||
              '';
            setSelectedCategory((prev) => prev || preferredTypeId);
