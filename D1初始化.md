...
| 15 | `music_playlist_songs` | 音乐歌单歌曲 |
| 16 | `player_settings` | 播放器设置（支持匿名/登录用户） |
| 17 | `skip_times` | 跳过时间（跨来源共享） |
...
# 查看播放器设置
SELECT * FROM player_settings WHERE user_id='anonymous';

# 查看跨来源跳过时间
SELECT * FROM skip_times ORDER BY updated_at DESC LIMIT 20;
```

## 新增功能表说明

### player_settings 表
用于存储播放器设置，支持匿名用户和登录用户的偏好设置同步。

| 字段 | 类型 | 说明 |
|------|------|------|
| `user_id` | TEXT | 用户名或 'anonymous' |
| `settings` | TEXT | JSON 格式的播放器设置 |
| `updated_at` | INTEGER | 更新时间戳 |

播放器设置包括：音量、播放速率、自动播放、弹幕开关、影院模式、Anime4K 超分设置等。

### skip_times 表
用于存储跳过片头片尾时间，支持同名视频跨来源共享。

| 字段 | 类型 | 说明 |
|------|------|------|
| `title_normalized` | TEXT | 标准化后的视频标题 |
| `intro_time` | INTEGER | 片头跳过时间（秒） |
| `outro_time` | INTEGER | 片尾跳过时间（秒） |
| `updated_at` | INTEGER | 更新时间戳 |
